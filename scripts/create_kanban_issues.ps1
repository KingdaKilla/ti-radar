...
$GH = "C:\Users\bensc\AppData\Local\gh-cli\bin\gh.exe"
$REPO = "KingdaKilla/ti-radar"
$PROJECT_NR = 2

Write-Host "=== Schritt 1: Labels erstellen ===" -ForegroundColor Cyan
...
foreach ($l in $labels) {
    $name = $l[0]; $color = $l[1]; $desc = $l[2]
    Write-Host "  Label: $name"
    & $GH label create $name --color $color --description $desc --repo $REPO 2>$null
}

Write-Host ""
Write-Host "=== Schritt 2: 50 Issues erstellen ===" -ForegroundColor Cyan
...
$issues = @(
    ...
    @("use-case", "S-Curve-Fitting-Modul entwickeln", "domain/scurve.py mit Logistik- und Gompertz-Funktionen, fit_best_model() fuer automatische Modellselektion. Levenberg-Marquardt via scipy.optimize.curve_fit."),
...
foreach ($issue in $issues) {
    ...
    Write-Host "  [$count/50] $title"
    $url = & $GH issue create --title $title --body $body --label $label --repo $REPO 2>&1
    $issueUrls += $url
    Write-Host "    -> $url"
...
Write-Host ""
Write-Host "=== Schritt 3: Issues dem Project Board zuweisen ===" -ForegroundColor Cyan

$assigned = 0
foreach ($url in $issueUrls) {
    if ($url -match "https://") {
        $assigned++
        Write-Host "  [$assigned] $url"
        & $GH project item-add $PROJECT_NR --owner KingdaKilla --url $url 2>&1 | Out-Null
        Start-Sleep -Milliseconds 300
    }
}

Write-Host ""
Write-Host "=== Fertig! $count Issues erstellt, $assigned dem Board zugewiesen ===" -ForegroundColor Green
